    ...
    public class JsonMapper {
        ...
        private static void RegisterBaseExporters() {
            ...
            base_exporters_table[typeof(DateTime)] =
                delegate(object obj, JsonWriter writer) {
                    try {
                        writer.Write(Convert.ToString(DateTime.Parse(Convert.ToString(obj)),
                                                        datetime_format));

                        //writer.Write("01-01-01");
                    }
                    catch (Exception exDateTime) {
                        Debug.Log(exDateTime);
                        try {

                            // TODO clean up hack.
                            //Regex regex = new Regex(@"\/Date((.*?)-(.*?).)\/");
                            string objFormat = Convert.ToString(obj);//\/Date(1298708605000-0700)\/
                            string result = Regex.Replace(objFormat, @"\/Date((.*?)-(.*?).)\/", "$1");

                            //regex.Replace(objFormat, "$1");
                            LogUtil.Log("RESSULLLT:" + result);
                            writer.Write(Convert.ToString(DateTime.FromFileTimeUtc(Convert.ToInt64(result)),
                                                            datetime_format));
                        }
